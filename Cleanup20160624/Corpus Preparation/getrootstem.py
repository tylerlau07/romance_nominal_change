...
# print segmentWord('orientes')

# _cont if continuing because of timeout
reader = codecs.open('noun_paradigms_cont.txt', encoding='utf-8', mode='rU')

with open('rootending.txt', mode = 'ab') as f:
	out = csv.writer(f, delimiter = '\t')
	# Don't write following if continuing
	# out.writerow(["Form", "Root", "Suffix", "Case", "Gender", "Declension", "Parent", "English", "Total", "Prose", "Poetry"])
	# Each row is a word's case forms
	for row in reader.readlines()[1:]:
		row = row.strip("\r\n").split("\t")
		# First and second elements are gender and declension
		lat, eng, dec, gender, total, prose, poetry = row[0:7]
		
		# Each element in the row is a word
		for i in range(7, len(row)):
			if "/" in row[i]:
				word = row[i][:row[i].index("/")]
			else:
				word = row[i]
			case = cases[i-7]
			...
			to_write = word, re_root, re_suffix, case, gender, dec, lat, eng, total, prose, poetry
			print word, to_write
