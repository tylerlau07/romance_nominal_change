...
# Original Info
file_originfo <- read_delim(files[1], delim = '\t')
...
orig_counts <- c(as.list(table(o_gen_info)), 
                 ...
                 #
                 as.list(table(paste0(o_gen_info, o_case_info))),
                 as.list(table(paste0(o_dec_info, o_case_info))))

### Create list of counts for each trial
total_counts <- list()
for (file in files){
  read <- read_delim(file, delim = '\t')
  if (gen_drop == TRUE) {read <- read[!grepl(":gen", read$`Declined Noun`), ]}
  trial_no <- substring(file, regexpr("Trial", file)+5, regexpr(".txt", file)-1)
  counts <- list()
  for (i in 1:(ncol(read)-2)) {
    ...
    # Gender + CaseNum TO Gender + CaseNum counts
    counts[[gen_number]] <- c(counts[[gen_number]], as.list(table(paste0(o_gen_info, o_case_info, "TO", c_gen_info, c_case_info))))
    # Declension + Case + Number counts
    counts[[gen_number]] <- c(counts[[gen_number]], as.list(table(paste0(o_dec_info, o_case_info, "TO", c_dec_info, c_case_info))))
    # Declension + Case + Number TO Case + Number counts
    counts[[gen_number]] <- c(counts[[gen_number]], as.list(table(paste0(o_dec_info, o_case_info, "TO", c_case_info))))
  }
...
plot_var <- "mnomsgTOfnomsg"

ggplot(df_percents, aes(x=Generation, y=mnomsgTOfnomsg))+
...
for (file in files){
  ...
  df_end$endcounts[names(table(paste(c_gen_info, c_dec_info, c_case_info))), trial_no] <- as.numeric(table(paste(c_gen_info, c_dec_info, c_case_info)))
}
